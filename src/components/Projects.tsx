              ...
              <div className="mt-auto">
                {p.tools && p.tools.length>0 && (
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {p.tools.map((t,j)=><span key={j} className="text-xs px-2 py-0.5 bg-cyan-500/10 text-cyan-400 rounded border border-cyan-500/20">{t}</span>)}
                  </div>
                )}
                {p.tags && p.tags.length>0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {p.tags.map((t,j)=><span key={j} className="text-xs px-2 py-0.5 bg-slate-700/60 text-slate-400 rounded border border-slate-600/50">{t}</span>)}
                  </div>
                )}
              </div>
            </div>
