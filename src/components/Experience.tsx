                ...
                <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6 hover:border-cyan-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-cyan-500/5">
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-4">
                    <div>
                      ...
                      {e.department && (
                        <p className="text-slate-400 text-xs mt-0.5">{e.department}</p>
                      )}
                      {e.location && (
                    ...
                    <div className="flex flex-col items-start md:items-end gap-1.5 shrink-0">
                      ...
                      {e.type && (
                        <span className="text-xs px-3 py-1 bg-blue-500/15 text-blue-400 rounded-full border border-blue-500/25">{e.type}</span>
                      )}
                    </div>
                  </div>
                  ...
                  {e.tags && e.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {e.tags.map((t, j) => (
                        <span key={j} className="text-xs px-2 py-1 bg-slate-700/60 text-slate-400 rounded-md border border-slate-600/50">
                          {t}
                        </span>
                      ))}
                    </div>
