...
const colorConf: Record<string, { iconBg: string; icon: string; border: string }> = {
  cyan:   { iconBg: 'bg-cyan-500/10',   icon: 'text-cyan-400',   border: 'border-cyan-500/20' },
  indigo: { iconBg: 'bg-indigo-500/10', icon: 'text-indigo-400', border: 'border-indigo-500/20' },
  amber:  { iconBg: 'bg-amber-500/10',  icon: 'text-amber-400',  border: 'border-amber-500/20' },
};
              ...
              <StaggerContainer className="flex flex-wrap gap-2" staggerDelay={0.04}>
                {skills.domains.map((domain) => (
                  <StaggerItem key={domain}>
                    <span className="inline-flex px-3 py-1.5 rounded-lg border border-slate-700/60 bg-slate-800/40 text-xs text-slate-300 font-medium hover:border-cyan-500/30 hover:text-cyan-400 transition-colors duration-200 cursor-default">
                      {domain}
                    </span>
                  </StaggerItem>
                ))}
            ...
            <div className="glass-card p-6">
              ...
              <div className="flex flex-wrap gap-3">
                {[
                  ...
                  <div key={lang} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-800/50 border border-slate-700/40">
