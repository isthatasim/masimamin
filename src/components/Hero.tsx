  ...
  useEffect(() => {
    ...
    if (!document.getElementById(id)) {
      ...
      s.textContent = `
        ...
        @keyframes glow-ring   { 0%,100%{box-shadow:0 0 30px rgba(6,182,212,.4),0 0 60px rgba(6,182,212,.15)}
                                 50%    {box-shadow:0 0 55px rgba(6,182,212,.7),0 0 90px rgba(6,182,212,.3)} }
      `;
      document.head.appendChild(s);
    }
    return () => { document.getElementById('hero-float-style')?.remove(); };
  }, []);

  const imgSrc = personal.image ?? null;

  return (
    <section id="home" className="relative min-h-screen flex items-center overflow-hidden">

      {/* Subtle background glow orbs — behind avatar, above canvas */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute w-[520px] h-[520px] rounded-full"
          style={{ background:'radial-gradient(circle,rgba(6,182,212,.08) 0%,transparent 70%)', top:'-8%', right:'5%' }} />
        <div className="absolute w-[320px] h-[320px] rounded-full"
          style={{ background:'radial-gradient(circle,rgba(59,130,246,.06) 0%,transparent 70%)', bottom:'15%', left:'5%' }} />
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-14 items-center">
          ...
          <div className="order-2 lg:order-1">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {[
                {label:'PhD Researcher',     c:'rgba(6,182,212,.12)',  bc:'rgba(6,182,212,.4)',  t:'#67e8f9'},
                {label:'University of Genova',c:'rgba(59,130,246,.12)',bc:'rgba(59,130,246,.4)', t:'#93c5fd'},
                {label:'Horizon Europe CLOE', c:'rgba(139,92,246,.12)',bc:'rgba(139,92,246,.4)', t:'#c4b5fd'},
              ].map(b=>(
                <span key={b.label} className="text-xs px-3 py-1 rounded-full font-medium"
                  style={{background:b.c,border:`1px solid ${b.bc}`,color:b.t}}>
                  {b.label}
                </span>
              ))}
            </div>

            <p className="text-xs font-bold tracking-[.3em] uppercase mb-2" style={{color:'#06b6d4'}}>
              Hello, I'm
            </p>
            <h1 className="font-extrabold text-white leading-[1.08] mb-4"
            ...
            {personal.summary && (
              <p className="text-sm leading-relaxed mb-7 max-w-xl" style={{color:'#94a3b8'}}>
                {personal.summary.split('\n')[0].slice(0,200)}{personal.summary.length>200?'…':''}
              </p>
            )}
            ...
            <div className="flex flex-wrap gap-2 mb-7">
              {['Reinforcement Learning','Smart Grids','Energy Communities','Federated Learning'].map(t=>(
                <span key={t} className="text-xs px-3 py-1 rounded-full"
                  style={{background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.15)',color:'#cbd5e1'}}>
                  {t}
                </span>
              ))}
            </div>
            ...
            <div style={{animation:'float-hero 6s ease-in-out infinite'}}>
              <div className="relative">
                ...
                <div className="absolute inset-0 rounded-full scale-[1.2] blur-3xl"
                  style={{background:'radial-gradient(circle,rgba(6,182,212,.3) 0%,rgba(59,130,246,.15) 50%,transparent 70%)'}} />
                {/* Circle */}
                <div className="relative overflow-hidden rounded-full"
                  style={{
                    width:'clamp(260px,32vw,330px)',height:'clamp(260px,32vw,330px)',
                    border:'3px solid rgba(6,182,212,.75)',
                  ...
                  <div className="absolute inset-0" style={{background:'linear-gradient(135deg,#060d1e,#0a1628)'}}>
                    <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
                      <g stroke="rgba(6,182,212,.22)" strokeWidth="0.5" fill="none">
                        ...
                        <line x1="35" y1="12" x2="45" y2="58"/> <line x1="8"  y1="68" x2="25" y2="88"/>
                        <line x1="55" y1="8"  x2="68" y2="52"/> <line x1="25" y1="88" x2="45" y2="58"/>
                      </g>
                  ...
                  {/* Portrait */}
                  {imgSrc ? (
                    <img src={imgSrc} alt={personal.name}
