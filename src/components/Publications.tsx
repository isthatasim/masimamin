  ...
  return (
    <div className="bg-slate-800/40 border border-slate-700/50 rounded-lg p-5 hover:border-cyan-500/30 transition-all duration-200">
      <div className="flex gap-3">
        <span className="text-cyan-400 font-bold text-sm shrink-0 mt-0.5">
          [{index + 1}]
        </span>
        <div className="flex-1 min-w-0">
          <p className="text-white font-medium text-sm leading-relaxed mb-1.5">{pub.title}</p>
          {pub.authors && (
            <p className="text-slate-400 text-xs mb-1">{pub.authors}</p>
          )}
          <div className="flex flex-wrap items-center gap-2">
            {pub.venue && (
              <span className="text-slate-300 text-xs italic">{pub.venue}</span>
            )}
            {pub.year && (
              <span className="text-slate-500 text-xs">({pub.year})</span>
            )}
            {pub.vol && (
              <span className="text-slate-500 text-xs">{pub.vol}</span>
            )}
            {badge && (
          ...
          {pub.note && (
            <p className="text-slate-500 text-xs mt-1 italic">{pub.note}</p>
          )}
        </div>
      </div>
    </div>
...
function PubSection({ title, pubs, badge, badgeColor, emptyMsg }: {
  title: string;
  pubs: Pub[];
  badge?: string;
  badgeColor?: string;
  ...
  return (
    <section id="publications" className="py-20 bg-slate-950">
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="text-center mb-14">
          ...
          <div className="flex flex-wrap justify-center gap-4 text-sm">
            <div className="px-4 py-2 bg-slate-800/60 border border-slate-700/50 rounded-lg">
              <span className="text-2xl font-bold text-cyan-400">{totalPublished}</span>
              <p className="text-slate-400 text-xs">Peer-Reviewed</p>
            </div>
            <div className="px-4 py-2 bg-slate-800/60 border border-slate-700/50 rounded-lg">
              <span className="text-2xl font-bold text-amber-400">{journalsUnderReview.length}</span>
              <p className="text-slate-400 text-xs">Under Review</p>
            </div>
            <div className="px-4 py-2 bg-slate-800/60 border border-slate-700/50 rounded-lg">
              <span className="text-2xl font-bold text-purple-400">{selectedConferences.length}</span>
              <p className="text-slate-400 text-xs">Conferences</p>
            </div>
            <div className="px-4 py-2 bg-slate-800/60 border border-slate-700/50 rounded-lg">
              <span className="text-2xl font-bold text-green-400">{workingPapers.length}</span>
        ...
        <PubSection
          title="Under Review / In Press"
          pubs={journalsUnderReview}
          badge="Under Review"
