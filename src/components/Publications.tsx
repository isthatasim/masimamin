...
type Pub = {
  ...
  venue?: string; journal?: string; booktitle?: string; conference?: string;
  year?: string | number; date?: string;
...
function PubCard({ pub, idx }: { pub: Pub; idx: number }) {
  ...
  return (
    <div className="card hover:border-blue-500/30" style={{ padding: "0.875rem 1.125rem" }}>
      <div className="flex gap-3">
        <div className="flex-shrink-0 w-6 h-6 rounded bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400 font-bold" style={{ fontSize: "0.6rem" }}>
          {String(idx + 1).padStart(2, "0")}
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="text-slate-200 font-medium text-sm leading-snug mb-1.5">
            {href
              ? <a href={href} target="_blank" rel="noopener noreferrer" className="hover:text-cyan-400 transition-colors">{pub.title || pub.name}</a>
              : (pub.title || pub.name)
            }
          </h3>
        ...
        <div className="flex flex-wrap items-end justify-between gap-4 mb-12">
          ...
          <div className="flex gap-2 flex-wrap">
            <a href="https://scholar.google.com/citations?user=YOURID" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 tag tag-cyan">
              <span className="font-mono text-xs">GS</span> Google Scholar
            </a>
            <a href="https://www.researchgate.net/profile/YOURPROFILE" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 tag tag-green">
              <span className="font-mono text-xs">RG</span> ResearchGate
            </a>
          </div>
        </div>
